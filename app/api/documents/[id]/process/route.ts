...
async function setupGoogleCredentials() {
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred'
    console.error("❌ Failed to set up Google credentials:", errorMessage)
    throw new Error(`Failed to set up Google credentials: ${errorMessage}`)
  }
}
  ...
  try {
    ...
    while (attempts < maxAttempts) {
      ...
      try {
        ...
        const errorMessage = attemptError instanceof Error ? attemptError.message : 'Unknown attempt error'
        console.log(`processWithGoogleDocumentAI: Attempt ${attempts} failed:`, errorMessage)
    ...
    if (Object.keys(extractedData).length === 0 && ocrText) {
      ...
      lines.slice(0, 50).forEach((line: string, index: number) => { // First 50 lines
    ...
    console.error("processWithGoogleDocumentAI: Error:", errorMessage)
    console.error("processWithGoogleDocumentAI: Error stack:", errorStack)
    throw new Error(`Google Document AI processing failed: ${errorMessage}`)
  }
}
